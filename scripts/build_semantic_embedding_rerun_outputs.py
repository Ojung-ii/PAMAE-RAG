...
def build_outputs(
    *,
        ...
        pool_rows.append(
            {
                ...
                "answer_on_support_tree": bool(answer_ids & {node_id(nodes, idx) for idx in pool.support_tree_chunks}),
                "answer_in_shell1": bool(answer_ids & {node_id(nodes, idx) for idx in pool.shell1_chunks}),
                "answer_in_shell2": bool(answer_ids & {node_id(nodes, idx) for idx in pool.shell2_chunks}),
            }
        )
    ...
    (root / "semantic_attribution_metrics.json").write_text(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build semantic embedding rerun attribution outputs.")
